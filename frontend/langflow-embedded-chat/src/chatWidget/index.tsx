...
export default function ChatWidget({
  ...
  const sessionId = useRef(session_id ?? uuidv4());
    ...
    // Add event dispatch in the updateLastMessage function
    function updateLastMessage(message: ChatMessageType) {
      setMessages((prev) => {
        prev[prev.length - 1] = message;
        const updatedMessages = [...prev];
        // Dispatch a custom event and pass the updated message
        console.log('[Langflow] Dispatch update event', { updatedMessages, message });
        window.dispatchEvent(new CustomEvent('langflow:messages:updated', {
          detail: {
    ...
    // Add event dispatch in the addMessage function
    function addMessage(message: ChatMessageType) {
      setMessages((prev) => {
        const newMessages = [...prev, message];
        // Dispatch a custom event and pass all messages
        // console.log('[Langflow] Dispatch add event', { newMessages, message });
        window.dispatchEvent(new CustomEvent('langflow:messages:added', {
          detail: {
